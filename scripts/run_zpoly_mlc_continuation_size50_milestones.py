...
EPISODES = 50
TRAIN_UPDATES = 2000
MILESTONE_STEPS = (400, 800, 1200, 1600, 2000)
BATCH_SIZE = 32
D_MODEL = 192
LAYERS = 2
HEADS = 4
...
def continuation_study_bank() -> dict[str, list[dict[str, Any]]]:
    """Checker-verified two-step studies: root macro, then one primitive cleanup.

    The macro remains at root in every study, preserving the original MLC study
    position. The only new information is a post-macro continuation transition.
    Payloads are chosen to avoid containing any accepted macro themselves.
    """
    macros = {m['abstraction_id']: m for m in accepted_macros()}
    x, y, z = base.V('x'), base.V('y'), base.V('z')
    payloads = [
        x, y, z,
        base.Add(x, y), base.Add(y, z), base.Add(z, x),
        base.Mul(x, y), base.Mul(y, z), base.Mul(z, x),
        base.Add(base.Mul(x, y), z),
        base.Mul(base.Add(x, y), z),
        base.Add(x, base.Mul(y, z)),
    ]
    out: dict[str, list[dict[str, Any]]] = defaultdict(list)
    for mid, macro in macros.items():
        for i, payload in enumerate(payloads):
            if mid == 'abs_v0_000':
                # Macro A -> (payload + 0), then ADD_ZERO_R -> payload.
                env = {'A0': base.Add(copy.deepcopy(payload), base.C(0))}
                second_action = 'ADD_ZERO_R'
            elif mid == 'abs_v0_001':
                # Macro B -> (payload * 1), then MUL_ONE_R -> payload.
                env = {'A0': base.Mul(copy.deepcopy(payload), base.C(1))}
                second_action = 'MUL_ONE_R'
            elif mid == 'abs_v0_002':
                # Macro C -> (1 * payload), then MUL_ONE_L -> payload.
                env = {'A0': base.Mul(base.C(1), copy.deepcopy(payload))}
                second_action = 'MUL_ONE_L'
            elif mid == 'abs_v0_003':
                # Macro D -> (0 + payload), then ADD_ZERO_L -> payload.
                env = {
                    'A0': copy.deepcopy(payloads[(i + 5) % len(payloads)]),
                    'A1': base.Add(base.C(0), copy.deepcopy(payload)),
                }
                second_action = 'ADD_ZERO_L'
            else:
                raise ValueError(mid)

            start = base.instantiate_pattern(macro['lhs_pattern'], env)
            intermediate = base.apply_macro_action(start, macro)
            if intermediate is None:
                raise AssertionError((mid, 'macro_not_applicable'))
            ok, reason = curve.checker_verify_macro_instance(start, intermediate, macro)
            if not ok:
                raise AssertionError((mid, reason))
            final = base.apply_primitive_action(intermediate, second_action)
            if final is None or not base.same_ast(final, payload):
                raise AssertionError((mid, second_action, intermediate, final, payload))

            # Ensure the intended macro really executes at root, not in a nested payload.
            paths = [
                path for path in base.iter_paths(start)
                if base.match_pattern(macro['lhs_pattern'], base.get_at_path(start, path)) is not None
            ]
            if paths != [()]:
                raise AssertionError((mid, 'macro_not_root_unique', paths))

            out[mid].append({
                'expr': start,
                'goal': final,
                'action_name': mid,
                'demo_kind': 'macro_then_primitive',
                'continuation_trace': {
                    'macro_action': mid,
                ...
                'checker': {
                    'pass': True,
                    'macro_step_verified': True,
                    'second_primitive_verified': True,
                    'macro_at_root_only': True,
                },
            })
    return dict(out)
...
def build_episodes() -> tuple[list[dict[str, Any]], dict[str, dict[str, list[dict[str, Any]]]]]:
    root_studies = root_study_bank()
    continuation_studies = continuation_study_bank()
    studies = {'root': root_studies, 'continuation': continuation_studies}
    qrows = query_pool()
    ...
    for i in range(EPISODES):
        ...
        # Exactly one continuation demonstration per episode; remaining studies
        # are the same single-macro root examples as MLC-v1.
        cont_bank = continuation_studies[m]
        root_bank = root_studies[m]
        cont = copy.deepcopy(cont_bank[(i * 5 + macros.index(m)) % len(cont_bank)])
        root_start = (i * 3 + macros.index(m)) % len(root_bank)
        demos = [cont] + [
            copy.deepcopy(root_bank[(root_start + j) % len(root_bank)])
            for j in range(nstudy - 1)
        ]
        ...
        episodes.append({
            'episode_id': f'mlc_cont_{i:03d}_{m}',
            'macro': m,
            'studies': demos,
            ...
            'checker': {
                ...
                'continuation_studies': sum(d.get('demo_kind') == 'macro_then_primitive' for d in demos),
            },
        })
...
def serialize_episode(ep: dict[str, Any], specs) -> list[int]:
    ...
    for demo in ep['studies']:
        if demo.get('demo_kind') == 'macro_then_primitive':
            tr = demo['continuation_trace']
            # First transition is encoded exactly like the old study example:
            # start -> intermediate. Then one explicit post-macro continuation
            # transition is appended, including the primitive action token.
            toks += [TOK_DEMO, base.TOK_EXPR]
            toks += base.ast_to_tokens(demo['expr'])
            toks += [base.TOK_GOAL]
            toks += base.ast_to_tokens(tr['intermediate'])
            toks += [TOK_DEMO_ANSWER, by_name[tr['second_action']].token_id, base.TOK_EXPR]
            toks += base.ast_to_tokens(tr['intermediate'])
            toks += [base.TOK_GOAL]
            toks += base.ast_to_tokens(tr['final'])
        else:
            toks += [TOK_DEMO, base.TOK_EXPR]
            toks += base.ast_to_tokens(demo['expr'])
            toks += [base.TOK_GOAL]
...
def make_eval_episode(studies, macro_name: str, expr, goal, specs, study_count: int = 3):
    roots = studies['root'][macro_name]
    conts = studies['continuation'][macro_name]
    # Use a continuation payload away from the simple x/y/z entries to avoid
    # copying the held-out query's literal variable payload.
    demos = [copy.deepcopy(conts[5])]
    demos += [copy.deepcopy(roots[j]) for j in range(study_count - 1)]
    return {
        'episode_id': f'eval_cont_{macro_name}',
        'macro': macro_name,
        'studies': demos,
...
def train(device):
    ...
    for step in range(1, TRAIN_UPDATES + 1):
        ...
        if step in MILESTONE_STEPS:
            structural_now = evaluate_structural(model, specs, studies, device)
            milestone_row = {
                'step': step,
                'structural_heldout_exact': structural_now['structural_heldout_exact'],
                'macro_selection_accuracy': structural_now['macro_selection_accuracy'],
                'completion_conditional_on_correct_selection': structural_now['completion_conditional_on_correct_selection'],
                'completion_conditional_n': structural_now['completion_conditional_n'],
                'tasks': structural_now['tasks'],
            }
            milestone_structural.append(milestone_row)
            print(json.dumps({'milestone_structural': milestone_row}, sort_keys=True), flush=True)
            model.train()
    return model, specs, episodes, studies, {
        'final_train_loss_mean_100': sum(losses[-100:]) / 100,
        'wall_seconds': time.perf_counter() - started,
        'router_health_log': health_log,
        'milestone_structural': milestone_structural,
    }
...
def main():
    ...
    ap.add_argument('--output', default=str(ROOT / 'runs/mlc_cont_size50/mlc_cont_size50.json'))
    args = ap.parse_args()
    ...
    dataset_audit = {
        ...
        'macro_counts': {m: sum(ep['macro'] == m for ep in episodes) for m in sorted(studies['root'])},
        ...
        'continuation_demo_per_episode': all(ep['checker']['continuation_studies'] >= 1 for ep in episodes),
        'continuation_demo_count': sum(ep['checker']['continuation_studies'] for ep in episodes),
    }
    ...
    out = {
        'schema_version': 'hpm.zpoly.mlc_continuation_size50.v1',
        'seed': SEED,
        'device': str(device),
        'mechanism': 'same-context studies_with_macro_then_primitive_continuation_plus_novel_query',
        ...
        'protocol': {
            ...
            'continuation_examples_per_episode': 1,
            'continuation_trace': 'root macro -> intermediate -> explicit second primitive -> final',
